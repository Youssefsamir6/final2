...
function LiveActivityFeedComponent() {
  ...
  const logs = React.useMemo(() => (socketLogs && socketLogs.length ? socketLogs.slice(0, 12) : []), [socketLogs]);
      ...
      <div className="max-h-72 overflow-y-auto space-y-2 p-2">
        {logs.length === 0 ? (
          <div className="p-3 rounded-md border bg-card text-muted-foreground">
            No live events yet — start the camera device to see real logs.
          </div>
        ) : (
          logs.map((log: any) => (
            <div key={log.id} className="flex items-center gap-3 rounded-md p-2 hover:bg-muted">
              ...
              <div className="flex-1 text-sm">
                ...
                <div className="text-xs text-muted-foreground">
                  {log.location} • {new Date(log.timestamp).toLocaleTimeString()}
                </div>
              </div>
              <div
                className={`text-xs font-semibold ${
                  log.status === "authorized" ? "text-green-600" : "text-destructive"
                }`}
              >
